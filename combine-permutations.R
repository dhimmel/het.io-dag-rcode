...
dirs <- list()
dirs$project <- '/home/dhimmels/Documents/serg/gene-disease-hetnet/'
...
for (perm.num in perm.nums) {
  ...
  path <- file.path(dirs$permnets, network.id, 'model', 'aurocs.txt')
  auroc.dfs[[network.id]] <- read.delim(path)
}

mean.df <- auroc.dfs[[1]]
mean.df$auroc <- apply(sapply(auroc.dfs, function(auroc.df) auroc.df$auroc), 1, mean)

path <- file.path(dirs$plots, 'AUROCs-permuted.txt')
